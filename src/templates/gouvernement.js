...
const GouvernementPage = ({ data }) => {
  ...
  return (
    <Layout>
        ...
        <article className="flex gap-20 col">
          <section className="flex flex-col gap-5">
            ...
            {personne.adressePostale && (
              <p className="text-base font-medium">Adresse pro:<br /> {personne.adressePostale}</p>
            )}
            {personne.remarqueCoordonnEs && (
              <p className="text-base font-medium">Remarque Coordonnées: {personne.remarqueCoordonnEs}</p>
            )}
          </section>
          <article className="flex flex-col gap-5">
            <section className="flex items-center gap-2">
              <h1 className="text-4xl font-bold page">{personne.prNom} {personne.nom}</h1>
              <figure className="">
            ...
            <h2 className="text-3xl font-semibold">{personne.statut && personne.statut.nom}</h2>
            ...
            <p className="text-lg font-medium">Nombre de Mandats: {personne.nombreDeMandats}</p>
            <p className="text-lg font-medium">Remarques: {personne.remarquesCommentaires}</p>
          </article>
        </article>
      </div>
      {attach && (
      ...
      <section>
        <h2 className="margin-top margin-bottom text-xl font-bold mt-10 mb-5">Attaché à cette personne :</h2>
            ...
            <li key={attachedPerson.id} className="flex items-center gap-4 mt-4">
              <Link to={`/attaches/${attachedPerson.url}`} className="flex flex-col items-center gap-4">
                ...
                <div className="text-center">
                  <p className="font-semibold">{attachedPerson.prNom} {attachedPerson.nom}</p>
